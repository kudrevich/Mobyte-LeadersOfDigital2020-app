...
class Requests {
  ...
  static Future<http.Response> sentProfile() async {
    ...
    return http
        .put(URL + '/user',
            body: body, headers: {HttpHeaders.authorizationHeader: Gtoken, 'Content-Type': 'application/json'})
        .timeout(Duration(seconds: 10))
        .catchError((_) => handleTimeOutException(),
            test: (e) => e is TimeoutException)
        .catchError((_) {
  ...
  static Future<http.Response> sentAvatar() async {
    String body = jsonEncode(<String, dynamic>{
      "user": {'avatar':"data:image/jpeg;base64," + base64Encode(MyApp.user.avatar)},
    });
    return http
        .put(URL + '/user',
            body: body, headers: {HttpHeaders.authorizationHeader: Gtoken, 'Content-Type': 'application/json'})
