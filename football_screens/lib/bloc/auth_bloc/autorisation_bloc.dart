...
class AutorisationBloc extends Bloc<AutorisationEvent, AutorisationState> {
    ...
    if (isConnetection) {
      ...
      if (response != null) {
        ...
        if (response.statusCode != 201) {
          ...
          //нет ошибки
          Navigator.of(myGlobals.globalKey.currentContext).pushReplacement(MaterialPageRoute(builder: (_)=>MainPage()));
  ...
  Stream<AutorisationState> register() async* {
      ...
      Helpers.showWarningDialog('Ошибка регистрации', 'Номер введён неправильно');
      return;
    }
    ...
    {
      Helpers.showWarningDialog('Ошибка регистрации', 'Введённые пароли не совпадают');
      return;
    }
    var isConnetection = await Helpers.checkInternetConnection();
    if (isConnetection) {
  ...
  Stream<AutorisationState> logout() async*{
      if(await UserRepository.hasPushToken())
       UserRepository.deletePushToken();
      Navigator.of(myGlobals.globalKey.currentContext).pushReplacement(MaterialPageRoute(builder: (_)=>AuthPage()));
      myGlobals.auth=true;
